    ...
    <div>
        ...
        <candy-modal title="Create Product" v-show="create" size="modal-md" @closed="create = false">
            <div slot="body">
                ...
                <div class="form-group">
                    <label for="redirectURL">URL Kategori</label>
                    <input type="text" id="redirectURL" class="form-control" v-model="model.url">
                    <span class="text-info" v-if="model.url">URL anda akan disanitasi menjadi: <code>{{ url }}</code></span>
                    <span class="text-danger" v-if="request.getError('url')" v-text="request.getError('url')"></span>
                </div>
            </div>
            <template slot="footer">
                <button type="button" class="btn btn-primary" @click="save">Buat Kategori</button>
            </template>
        </candy-modal>
    </div>
</template>
